...
Type objective_function<Type>::operator() ()
{
  ...
  Type mu_S_equil = (exp(beta) * ( w_k - (w_k-alpha_g)*exp(-M-F_equil) ) / (Type(1.0) - exp(-M-F_equil) - ro*exp(-M-F_equil) + ro*exp(-Type(2.0)*M-Type(2.0)*F_equil) ));
  Type mu_R_equil = exp(beta);
  Type mu_N_equil = mu_R_equil / (Type(1.0) - exp(-M-F_equil));
  Type mu_S0 = (exp(beta) * ( w_k - (w_k-alpha_g)*exp(-M-Type(0.0)) ) / (Type(1.0) - exp(-M-Type(0.0)) - ro*exp(-M-Type(0.0)) + ro*exp(-Type(2.0)*M-Type(2.0)*Type(0.0)) ));
  Type sum_S0 = 0;
  for(int i=0;i<n_i;i++){
